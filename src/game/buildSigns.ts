...
function canvasFace(w: number, h: number, paint: (ctx: CanvasRenderingContext2D, w: number, h: number) => void) {
...
export function buildSigns(mats: Mats) {
  const group = new THREE.Group();
  group.name = "signs";
  const half = RWY.length / 2;

  const loc = (letter: string) =>
...
export function buildHoldLines(mats: Mats) {
  const g = new THREE.Group();
  g.name = "hold";
