...
import { RWY, TAXI } from "./constants";
import { buildHoldLines, buildSigns } from "./buildSigns";
import { buildTown } from "./buildTown";
import type { Mats } from "./materials";

function plane(w: number, h: number, mat: THREE.Material, y: number) {
...
function tagLit(mat: THREE.MeshStandardMaterial, base: number) {
  mat.userData.baseEmissive = base;
  mat.emissiveIntensity = base;
  return mat;
}

function elevatedLight(color: number, tall = false) {
...
function insetLight(color: number) {
  const g = new THREE.Group();
...
export type WorldGroups = {
  root: THREE.Group;
  markings: Record<string, THREE.Group>;
  lights: Record<string, THREE.Group>;
  features: Record<string, THREE.Group>;
  sky: THREE.Mesh;
  towerSeat: THREE.Vector3;
  towerLookYaw: number;
  papiUnits: THREE.Group[];
  setLightLevel: (level: 1 | 2 | 3) => void;
  setPapi: (redFromRwy: number) => void;
};

export function buildAerodrome(mats: Mats): WorldGroups {
  const root = new THREE.Group();
  const half = RWY.length / 2;
  const hw = RWY.width / 2;

  const markings: Record<string, THREE.Group> = {};
  const lights: Record<string, THREE.Group> = {};
  const features: Record<string, THREE.Group> = {};
  const mk = (bag: Record<string, THREE.Group>, name: string) => {
    const g = new THREE.Group();
    g.name = name;
    bag[name] = g;
    root.add(g);
  ...
  const z0 = -tot / 2 + sw / 2;
  const paintThr = (ax: number, dir: number) => {
  ...
  const papiUnits: THREE.Group[] = [];
  const mkPapi = (dir: number) => {
  ...
  const setLightLevel = (level: 1 | 2 | 3) => {
  ...
  const setPapi = (redFromRwy: number) => {
    const paint = (unit: THREE.Group, red: boolean) => {
      const col = red ? 0xff2244 : 0xfff6c8;
      const lens = unit.userData.lensMat as THREE.MeshStandardMaterial;
      const halo = unit.userData.haloMat as THREE.MeshBasicMaterial;
      if (lens) {
