...
import { makeNightSky, makeSky } from "./textures";
...
export async function loadMaps() {
  ...
  const entries = await Promise.all(
    names.map(async (n) => [n, await loader.loadAsync(`/textures/${n}.jpg`)] as const),
  );
  const bag = Object.fromEntries(entries) as Record<(typeof names)[number], THREE.Texture>;
  return bag;
}

export function createMaterials(maps: Awaited<ReturnType<typeof loadMaps>>) {
  return {
    ...
    cloudStorm: new THREE.MeshLambertMaterial({
      color: 0x6a727c,
      emissive: 0x2a3038,
